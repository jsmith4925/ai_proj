...
def get_end_rect(grid):
    ...
    end_set = False
    while (not end_set):
        temp_end_x = random.randint(0,2) + int(((numpy.shape(grid))[0]-1)/2)
        temp_end_y = random.randint(0,2) + int(((numpy.shape(grid))[1]-1)/2)

        if grid[temp_end_x][temp_end_y] == 0:
            end_pos = [temp_end_x, temp_end_y]
            end_rect = pygame.Rect(MAP_PADDING+1+(temp_end_x*BLOCK_RES),
                        MAP_PADDING+1 + (temp_end_y*BLOCK_RES),
                        BLOCK_RES,
                        BLOCK_RES)
            end_set = True
