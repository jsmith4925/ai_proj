...
def getGrid():
    ...
    maze_grid = new_maze(columns, rows);
    ...
    # for z in range(10):
    #     grid[25][z + 10] = NodeType.wall
    #     grid[25][z + 30] = NodeType.wall
...
def getGrownSnake(snake_nodes, direction, grid):
    ...
    if direction == Direction.right:
        new_tail = SnakeNode(tail.x-1, tail.y)
    elif direction == Direction.left:
        new_tail = SnakeNode(tail.x+1, tail.y)
    elif direction == Direction.up:
        new_tail = SnakeNode(tail.x, tail.y+1)
    else:
        new_tail = SnakeNode(tail.x, tail.y-1)
...
def drawNode(x, y, grid, screen):
    if grid[x][y] == NodeType.snake_head:
        color = snake_color
    elif grid[x][y] == NodeType.food:
        color = food_color
...
# Left, forward, right nodes of snake
def getNeighboringNodes(snake_nodes, direction, grid):
    head = snake_nodes[0]
...
def getPredictedDirection(snake_nodes, absolute_direction, model, inputs, grid, shuffle_predictions):
    ...
    for relative_direction in relative_directions:
        prediction = model.predict(np.array(
            [[inputs[0][0], inputs[0][1], inputs[0][2], inputs[1], relative_direction]]))
        if prediction > 0.9:
            break
        no_match_found = True
...
def generateFood(grid):
    food_position = (columns//2, rows//2)
...
def runGame(death_count, running_score, font, model):
    ...
    # while isGameOver(snake_nodes, grid) or not isGameOver(snake_nodes, grid):
    while True:
        score_count = 0
        # Update score
        game_stats_label = font.render("Maze #: {} Score: {}".format(
            death_count, running_score), 1, (0, 200, 255))
        ...
        shuffle_predictions = (
            stuck_position[snake_nodes[0].x][snake_nodes[0].y] > 5)
        ...
        direction, relative_direction = getPredictedDirection(
            snake_nodes, direction, model, inputs, grid, shuffle_predictions)

        previous_distance_between_snake_and_food = distanceBetweenSnakeAndFood(
            snake_nodes, food_position)
        snake_nodes = advanceSnake(snake_nodes, direction, grid)
        current_distance_between_snake_and_food = distanceBetweenSnakeAndFood(
            snake_nodes, food_position)
        ...
        if isGameOver(snake_nodes, grid):
            target_output = -1
        elif current_distance_between_snake_and_food >= previous_distance_between_snake_and_food:
            target_output = 0
        else:
            target_output = 1

        output = getOutputForTraining(
            target_output, inputs, snake_nodes, getRelativeDirection(current_direction, direction))
...
while(running):
    death_count += 1
    running_score += score_count
    runGame(death_count, running_score, font, model)
